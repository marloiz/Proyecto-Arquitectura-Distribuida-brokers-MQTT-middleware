...
public class ProtocoloInteraccionMqtt implements ProtocoloInteraccion {
	...
	//private ProtocoloSTP protocoloSTP = new ProtocoloSTP(gestorRecursos, mutex, sec, inforInput);
	...
	// public ArrayList<inforSTP> brokersMQTTExternos_STP = new
	// ArrayList<inforSTP>();
	...
	public Boolean iniciarComunicacion(InterceptPublishMessage pubMsg, String tipoAccion) {
		...
		if (pubMsg != null) { // gestiono un PUBLISH
			...
			// String portTransmisor = pubMsg.getServerURI();

			String[] parts = idTransmisor.split("-"); // Para ver si viene de proxy
			...
			try {
				...
				if (name.equalsIgnoreCase("STP") && idProxy.equalsIgnoreCase("Proxy")) {
					// Necesito procesar todos los mensajes de STP, ya sean de puerdos designados, root o bloqueados
					mutex.acquire();
					System.out.println("");
					...
					if (!puertoBLOQUEADO){
						...
						// System.out.println("ServerURI: " + portTransmisor);
	...
	public void STP(String _idTransmisor, String _content) {
		...
		Broker brokerEmisor = new Broker(_idTransmisor, parts[0], parts[1], Integer.parseInt(parts[2]), parts[3],
				parts[4], parts[5], Integer.parseInt(parts[6]), Long.valueOf(parts[7]));
		Long TInicio = Long.valueOf(parts[8]);
		protocoloSTP.algoritmoSTP(brokerEmisor, TInicio);
