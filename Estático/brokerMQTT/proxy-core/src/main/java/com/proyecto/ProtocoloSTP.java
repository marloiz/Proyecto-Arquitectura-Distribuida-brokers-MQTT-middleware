...
public class ProtocoloSTP {
	...
	public ArrayList<InforBPDU> inforBPDU = new ArrayList<InforBPDU>(); // Lista que guarda la informacion recibida en los BPDU

	public ArrayList <InforRTT> listaRTT = new ArrayList <InforRTT>();
	...
	public ProtocoloSTP(GestorRecursos _gestorRecursos, Semaphore _mutex, Boolean _sec, InforInput _inforInput, ArrayList <InforRTT> _listaRTT) {
	...
	public void generacionPUBLISH() {
		...
		if (!puertos_DESIGNADOS.isEmpty()) { // Si la lista de Proxys no esta vacia
			...
			String[] msg = new String[] { miBroker.my_port, miBroker.my_address, String.valueOf(miBroker.my_C),
					miBroker.root_ID, miBroker.root_port, miBroker.root_address, String.valueOf(miBroker.root_C), String.valueOf(miBroker.RTT) };
			...
			for (int i = 0; i < puertos_DESIGNADOS.size(); i++) {
				//String broker = ((String) inforInput.brokersMQTTExternos.get(i)).split(" ")[1]; // CUIDAO
	...
	public void algoritmoSTP (Broker brokerEmisor) {

		// Calculo el RTT del camino
		int RTT_enlace = mido_RTT(brokerEmisor);
		brokerEmisor.RTT = brokerEmisor.RTT + RTT_enlace;
		System.out.println("El RTT del camino total es: " + brokerEmisor.RTT);
		...
		if (miBroker.root_C == brokerEmisor.root_C) {
			// Ambos brokers ROOT tienen la misma capacidad, elegimos al broker que tenga menor ID
			...
			if (Integer.parseInt(id_myRoot[1]) == Integer.parseInt(id_rootEmisor[1])) { 	
				...
				if (brokerROOT.RTT > brokerEmisor.RTT){ // Tarda menos por su camino
					...
					// aqui que hago con el anterior ROOTPORT
				...
				if (brokerROOT.RTT == brokerEmisor.RTT) { 
					...
					if (brokerROOT.my_C == brokerEmisor.my_C){
						String[] id_my = brokerROOT.my_ID.split("-");
		...
		if (!brokerROOT.root_port.equalsIgnoreCase(miBroker.my_port) && !brokerROOT.my_port.equalsIgnoreCase(brokerEmisor.my_port) && miBroker.root_port.equalsIgnoreCase(brokerEmisor.root_port)) {
			...
			int lo_que_me_cuesta_enviarle_el_mensaje =  RTT_enlace + brokerROOT.RTT;
	...
	public int mido_RTT (Broker _brokerEmisor){
		// Valor del RTT
		int RTT = 0;

		for (int i = 0; i < listaRTT.size(); i++) {

			if(listaRTT.get(i).port.equalsIgnoreCase(_brokerEmisor.my_port) ) {
				RTT = listaRTT.get(i).RTT;
				break;
			}
	...
	public void reenvio_informacion (Broker _brokerEmisor) {
		...
		if (!puertos_DESIGNADOS.isEmpty()) { // Si la lista de puertos DESIGNADOS no esta vacia
			// Contenido del primer mensaje
			String[] msg = new String[] {miBroker.my_port, miBroker.my_address, String.valueOf(miBroker.my_C),
					_brokerEmisor.root_ID, _brokerEmisor.root_port, _brokerEmisor.root_address, String.valueOf(_brokerEmisor.root_C), String.valueOf(_brokerEmisor.RTT) };
			StringJoiner firstmessage = new StringJoiner(" ");
			for (String s : msg)
				firstmessage.add(s);
	...
	public void generoTimeOut(String port) {
		...
		ScheduledExecutorService newScheduler = Executors.newScheduledThreadPool(1); // creo 1 y lo agrego a la lista
		System.out.println("");
		...
		Runnable runnable = new Runnable() {
		    ...
		    int countdownStarter = 15; // Numero de segundos
