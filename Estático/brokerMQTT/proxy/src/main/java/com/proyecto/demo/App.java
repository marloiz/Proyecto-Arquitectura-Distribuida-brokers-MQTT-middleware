...
public class App {
	...
	public static Semaphore mutex = new Semaphore(1, true); // mutex para controlar el acceso a las listas de topics
															// (True=fair)
	public static ArrayList <InforRTT> listaRTT = new ArrayList <InforRTT>();
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		File ficheroRTT = new File("src/main/resources/config/RTT.txt");
		listaRTT = leerDatosRTT(ficheroRTT);
		...
		try {
			...
			protocoloSTP = new ProtocoloSTP(gestorRecursos, mutex, sec, inforInput, listaRTT);
		...
		try {
			...
			while (s.hasNextLine()) {
				...
				if (primeraLinea) {
					...
					System.out.println("Mi ID y mi puerto son " + _inforInput.ID + " y " + _inforInput.port + ". C: " + String.valueOf(_inforInput.C));					primeraLinea = false;
				} else {

					if (!parts[1].equals(_inforInput.port)) {
						_inforInput.brokersMQTTExternos.add("Proxy-" + String.valueOf(numBrokerExternos) + " " + parts[0] + ":" + parts[1]);
						numBrokerExternos++;
					} else {
	...
	public static ArrayList<InforRTT> leerDatosRTT(File fichero) {
		ArrayList<InforRTT> lista = new ArrayList<InforRTT>();
		...
		try {
			...
			while (s.hasNextLine()) {
				...
				parts = linea.split(" ");
				InforRTT _inforRTT = new InforRTT(parts[0], Integer.parseInt(parts[1]));
				lista.add(_inforRTT);
			}

		} catch (Exception e) {
			System.out.println("Error de lectura de datos en el RTT, excepcion: " + e.getClass().getName() + "\n" + "Mensaje: "
					+ e.getMessage());
		} finally {
			// En el finally cerramos el fichero,tanto si la lectura ha sido correcta o no
			try {
		...
		for (int i = 0; i < lista.size(); i++) {
			...
			System.out.println("Port:  " + lista.get(i).port + "     RTT: " + lista.get(i).RTT);
		}
		return lista;
	}
